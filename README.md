...
- **Авто-отклики hh.ru через API** — прямые POST на `/applicant/vacancy_response/popup`,
  ~1 сек на отклик (вместо ~60 сек Playwright)
- **Авто-отклики Habr** — Playwright (нет открытого API)
- **Pre-sync** — перед каждым циклом качаем список уже-отправленных откликов с hh
  и помечаем в БД, не тратим время на повторы
- **Сопроводительные** — Claude генерирует уникальное письмо под вакансию
- **Ответы на вопросы работодателя** — AI читает вопрос, заполняет ответ
- **Уведомления рекрутеров** — отслеживает hh.ru, Хабр и личку Telegram (через user-bot)
- **AI-ответы на сообщения** — генерирует естественный ответ без HR-штампов
- **Авто-поднятие резюме** — каждые 4 часа
- **Login health check** — каждые 30 мин проверяет сессии, при разлоге паузит
  auto_apply и шлёт критическое уведомление
