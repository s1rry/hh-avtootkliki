...
> ⚠️ **Только для личного использования.** Проект сделан для собственного поиска работы. Автоотклики нарушают правила hh.ru и Хабр Карьеры — используйте на свой риск.

## Поддержать автора

Если бот пригодился, можно поддержать развитие:

- **TON / USDT (сеть TON):** `UQDCmkc57YsmcAY0b10os1Ho-9K_TO7ub3RNldzxTlr1auNv`
- **USDT (сеть TRON, TRC20):** `TMSGFkrPx65W4Z5ckebeuH8kBQDR1rtfBb`

В самом боте: ⚙️ Настройки → 💎 Поддержать автора (адреса с копированием в один тап и кнопка перевода TON).
...
- **Rule-based анализ без AI** — мгновенный скоринг вакансий 0-100 по заголовку, ключевым словам,
  зарплате, формату и уровню, без трат токенов. Под какую профессию, ключевые слова и зарплату
  искать — задаётся в настройках, см. [Настройка под себя](#настройка-под-себя)
...
Чтобы бот искал и откликался под твою профессию (по умолчанию — аналитик):

1. **Должность и зарплата** — в файле `.env`:
   - `DESIRED_POSITION` — твоя роль (идёт в письма и в скоринг)
   - `DESIRED_SALARY_MIN`, `DESIRED_SALARY_MAX` — вилка
2. **Поисковые запросы** — список в `app/workers/vacancy_worker.py`: замени фразы вроде
   «аналитик BPMN», «аналитик SQL» на запросы своей профессии.
3. **Ключевые слова для скоринга** — в `app/ai/rule_analyzer.py`: плюс-слова стека и
   жёсткие минусы (что отсекать). Подставь свой стек.
4. **Резюме** — положи свой текст в `configs/resume.txt`. По нему AI отвечает на вопросы
   работодателя и проходит тесты. Файл локальный, в репозиторий не попадает.
5. Перезапусти бота: `systemctl restart job-hunter` (или перезапуск процесса).

## Безопасность

- **Логины и пароли** (hh, Хабр) и все токены хранятся только в локальном файле `.env`
  на твоём компьютере или сервере. Этот файл в `.gitignore` и **никогда не попадает
  в репозиторий**.
- Бот использует их **только** чтобы войти на hh.ru и Хабр от твоего имени. Никуда
  больше они не отправляются, не выгружаются и не передаются третьим лицам.
- **Резюме** (`configs/resume.txt`) тоже только локальное и в репозиторий не коммитится.
- Никакой телеметрии и аналитики на сторону нет: всё работает на твоей машине / VPS.
